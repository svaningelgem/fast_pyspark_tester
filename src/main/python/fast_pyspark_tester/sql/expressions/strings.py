...
from fast_pyspark_tester.sql.expressions.expressions import UnaryExpression, Expression
from fast_pyspark_tester.sql.types import StringType
from fast_pyspark_tester.utils import levenshtein_distance
...
class StringInStr(Expression):
    ...
    def __str__(self):
        return 'instr({0}, {1})'.format(self.column, self.substr)
