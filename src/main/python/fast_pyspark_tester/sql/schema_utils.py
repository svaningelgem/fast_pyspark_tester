...
from fast_pyspark_tester.sql.internal_utils.joins import INNER_JOIN, CROSS_JOIN, LEFT_JOIN,\
    LEFT_ANTI_JOIN, LEFT_SEMI_JOIN, RIGHT_JOIN, FULL_JOIN
from fast_pyspark_tester.sql.types import _infer_schema, _has_nulltype, _merge_type, \
...
def infer_schema_from_list(data, names=None):
    ...
    :return: :class:`fast_pyspark_tester.sql.types.StructType`
    """
    if not data:
