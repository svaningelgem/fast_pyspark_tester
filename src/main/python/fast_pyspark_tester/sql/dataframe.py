...
class DataFrame(object):
    ...
    def write(self):
        ...
        from fast_pyspark_tester.sql.readwriter import DataFrameWriter
    ...
    def show(self, n=20, truncate=True, vertical=False):
        ...
        |  2|Alice|
        +---+-----+
        >>> from fast_pyspark_tester.sql.functions import map_from_arrays, array, col
        >>> df = spark.range(3)
    ...
    def sample(self, withReplacement=None, fraction=None, seed=None):
        ...
        if not (is_withReplacement_set or is_withReplacement_omitted_kwargs or is_withReplacement_omitted_args):
            argtypes = [str(type(arg)) for arg in [withReplacement, fraction, seed] if arg is not None]
            raise TypeError(
    ...
    def sampleBy(self, col, fractions, seed=None):
        ...
        >>> from fast_pyspark_tester import Context
        >>> from fast_pyspark_tester.sql.session import SparkSession
        >>> from fast_pyspark_tester.sql.functions import count, lit
        >>> spark = SparkSession(Context())
    ...
    def join(self, other, on=None, how='inner'):
        ...
        >>> from fast_pyspark_tester import Context, Row
        >>> from fast_pyspark_tester.sql.session import SparkSession
        >>> from fast_pyspark_tester.sql.functions import length, col, lit
        >>> spark = SparkSession(Context())
        ...
        if how != CROSS_JOIN and on is None:
            raise IllegalArgumentException(
                'Join condition is missing. ' 'Use the CROSS JOIN syntax to allow cartesian products'
            )
    ...
    def sort(self, *cols, **kwargs):
        ...
        >>> from fast_pyspark_tester import Context, Row
        >>> from fast_pyspark_tester.sql.session import SparkSession
        >>> from fast_pyspark_tester.sql.functions import desc
        >>> spark = SparkSession(Context())
        >>> df = spark.createDataFrame(
        ...   [Row(age=5, name='Bob'), Row(age=2, name='Alice')]
        ... )
    ...
    def select(self, *cols):
        ...
        >>> from fast_pyspark_tester.sql.session import SparkSession
        >>> spark = SparkSession(Context())
        >>> from fast_pyspark_tester.sql.functions import (explode, split, posexplode,
        ...
        |  1|  b|a,b|1,2|a,b|
        +---+---+---+---+---+
        >>> from fast_pyspark_tester.sql.types import StructType, StructField, ArrayType, StringType
        >>> df = spark.createDataFrame(
        ...     [Row(a=[], b=None, c=[None])],
        ...     schema=StructType([
    ...
    def replace(self, to_replace, value=_NoValue, subset=None):
        ...
        if not any(
            all_of_type(rep_dict.keys()) and all_of_type(x for x in rep_dict.values() if x is not None)
    ...
    def _check_replace_inputs(self, subset, to_replace, value):
        ...
        if not isinstance(value, valid_types) and value is not None and not isinstance(to_replace, dict):
            raise ValueError(
        ...
        if not (subset is None or isinstance(subset, (list, tuple, str))):
            raise ValueError(
                'subset should be a list or tuple of column names, ' 'column name or None. Got {0}'.format(type(subset))
            )
        return value
    ...
    def withColumn(self, colName, col):
        ...
        >>> from fast_pyspark_tester import Context, Row
        >>> from fast_pyspark_tester.sql.session import SparkSession
        >>> from fast_pyspark_tester.sql.functions import length
        >>> spark = SparkSession(Context())
        >>> df = spark.createDataFrame(
        ...   [Row(age=5, name='Bob'), Row(age=2, name='Alice')]
        ... )
