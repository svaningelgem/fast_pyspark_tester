...
class Column(object):
    ...
    def asc(self):
        ...
        >>> from fast_pyspark_tester import Context, Row
        >>> from fast_pyspark_tester.sql.session import SparkSession
        >>> from fast_pyspark_tester.sql.functions import when, col
        >>> spark = SparkSession(Context())
    ...
    def alias(self, *alias, **kwargs):
        ...
        >>> from fast_pyspark_tester.sql.functions import map_from_arrays, array
    ...
    def cast(self, dataType):
        ...
        >>> from fast_pyspark_tester import Context, Row
        >>> from fast_pyspark_tester.sql.session import SparkSession
        >>> from fast_pyspark_tester.sql.types import StringType
        >>> spark = SparkSession(Context())
        >>> df = spark.createDataFrame(
        ...   [Row(age=2, name='Alice'), Row(age=5, name='Bob')]
        ... )
    ...
    def when(self, condition, value):
        ...
        >>> from fast_pyspark_tester.sql import functions as F
        >>> from fast_pyspark_tester import Context, Row
        >>> from fast_pyspark_tester.sql.session import SparkSession
        >>> spark = SparkSession(Context())
        >>> df = spark.createDataFrame(
    ...
    def otherwise(self, value):
        ...
        """
        if not isinstance(self.expr, CaseWhen):
            raise IllegalArgumentException('otherwise() can only be applied on a Column previously generated by when()')
    ...
    def is_an_aggregation(self):
        ...
        if isinstance(self.expr, str):
            return False
        raise NotImplementedError('Not implemented column expression type: {0}'.format(type(self.expr)))

    def output_fields(self, schema):
        if isinstance(self.expr, Expression):
            return self.expr.output_fields(schema)
        return [StructField(name=self.col_name, dataType=self.data_type, nullable=self.is_nullable)]
