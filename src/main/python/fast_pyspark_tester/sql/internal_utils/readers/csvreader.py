...
from fast_pyspark_tester.fileio import TextFile
...
class CSVReader(object):
    ...
    def read(self):
        ...
        rdd = rdd_filenames.flatMap(partial(parse_csv_file, partitions, partition_schema, self.schema, self.options))
        ...
        schema_with_string = StructType(fields=[StructField(field.name, StringType()) for field in schema.fields])
...
def csv_record_to_row(
    record, options, schema=None, header=None, null_value=None, partition_schema=None, partition=None,
):
    ...
    row = create_row(
        itertools.chain(field_names, partition_field_names), itertools.chain(record_values, partition or []),
    )
    return row
