...
from fast_pyspark_tester.sql.functions import parse
from fast_pyspark_tester.sql.types import row_from_keyed_values
...
class ColumnStatHelper(object):
    ...
    Its implementation for simple metrics differs from fast_pyspark_tester.stat_counter.StatCounter
    ...
    def get_stat(stats_counter, stat):
        if stat in ('count', 'mean', 'stddev', 'min', 'max'):
            ...
            try:
                ...
                raise ValueError(f'Unable to parse {stat} as a percentile') from None
